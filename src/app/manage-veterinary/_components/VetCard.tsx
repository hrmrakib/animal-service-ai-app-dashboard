  ...
  return (
    <div className="bg-[#FCFBF8] rounded-xl border border-[#F0EBE1] overflow-hidden flex flex-col transition-shadow hover:shadow-md">
      ...
      <div className="p-6 flex flex-col items-center text-center pb-4">
        ...
        <h3 className="font-bold text-gray-900 text-lg leading-tight">{vet.name}</h3>
        <p className="text-xs text-gray-500 mt-1">{vet.specialization.join(", ")}</p>
      </div>
      ...
      <div className="flex border-y border-[#F0EBE1] bg-[#FAF8F3]">
        <div className="flex-1 flex flex-col items-center justify-center py-3 border-r border-[#F0EBE1]">
          <span className="text-[#d08726] font-bold text-lg leading-tight">{vet.patients}</span>
          <span className="text-[10px] text-gray-500 font-medium uppercase tracking-wider mt-0.5">Patients</span>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center py-3 border-r border-[#F0EBE1]">
          <span className="text-[#d08726] font-bold text-lg leading-tight">{vet.experienceYears} Years</span>
          <span className="text-[10px] text-gray-500 font-medium uppercase tracking-wider mt-0.5">Experience</span>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center py-3">
          <span className="text-[#d08726] font-bold text-lg leading-tight">{vet.rating}</span>
          <span className="text-[10px] text-gray-500 font-medium uppercase tracking-wider mt-0.5">Rating</span>
        </div>
      </div>
      ...
      <div className="p-5 flex flex-col gap-4 flex-1">
        ...
        {/* Specialization Tags */}
        <div>
          <h4 className="text-xs font-semibold text-gray-900 mb-1.5">Specialization</h4>
          <div className="flex flex-wrap gap-1.5">
            {vet.specialization.map((spec, idx) => (
              <span key={idx} className="px-2 py-0.5 rounded-full border border-green-300 text-[10px] text-green-700 bg-green-50/50">
                {spec}
              </span>
            ))}
          </div>
        ...
        {/* Certifications Tags */}
        <div>
          <h4 className="text-xs font-semibold text-gray-900 mb-1.5">Certifications</h4>
          <div className="flex flex-wrap gap-1.5">
            {vet.certifications.map((cert, idx) => (
              <span key={idx} className="px-2 py-0.5 rounded-full border border-[#d08726]/40 text-[10px] text-[#d08726] bg-[#d08726]/5">
                {cert}
              </span>
            ))}
          </div>
        ...
        <div className="border-t border-[#F0EBE1] pt-3 mt-auto space-y-2">
          ...
          <div className="flex justify-between items-center text-xs">
            ...
            <span className="font-medium text-gray-900 text-right">{vet.clinic}</span>
          </div>
        </div>
      </div>
